#!/usr/bin/env node
/**
 * smoke-test.js — STM v2 parser smoke test
 *
 * Usage:
 *   node scripts/smoke-test.js <file.stm>
 *   node scripts/smoke-test.js examples/
 *
 * Parses one or more .stm files with the tree-sitter-stm v2 grammar and emits
 * a JSON summary of the constructs found. Exits with code 1 if any parse errors
 * are encountered.
 *
 * Requires: tree-sitter and tree-sitter-stm bindings to be built first.
...
const STM = require("../");          // tree-sitter-stm binding (built from grammar.js)
const fs = require("fs");
