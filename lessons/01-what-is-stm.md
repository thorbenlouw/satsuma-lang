# Lesson 01 — What Is STM & Your First File

STM stands for **Structured Transform Markup**. It is a compact language for describing data structures, field mappings, and transformation intent in one place. The goal is to replace fragile spreadsheet-based mapping documents with something that is readable by humans, useful to AI agents, and stable enough for parser-first tooling.

This lesson introduces the core idea behind STM and walks through your first working file. By the end, you will understand the delimiter model, the string and comment forms, and a minimal `schema` + `mapping` example.
...
STM puts these concerns into a single text artifact:
...
STM is built around one very simple rule:
...
If something contains other STM elements, it belongs in `{ }`.
...
STM does not force every transformation into a mini programming language. If plain English is the clearest expression of the rule, plain English is valid STM.

## The Four String Forms You Will See

STM uses a small set of quoting forms, each with a specific job.
...
STM has line comments only. Each form carries a different meaning.
...
Here is the smallest useful STM document: one source schema, one target schema, and one mapping arrow.
...
The `->` arrow is the most important operator in STM. Read it as “maps to”.
...
For now, the important thing is learning how STM separates shape, flow, and intent.

## A Slightly Smarter First Mapping

STM becomes more useful as soon as you add transformation intent.
...
Lesson 1 introduced the foundations of STM:

- STM replaces fragmented mapping documentation with a single structured artifact
- `( )`, `{ }`, and `" "` each have exactly one job
- STM uses four quoting forms: double quotes, triple double quotes, backticks, and single-quoted labels
- comments come in three types: `//`, `//!`, and `//?`
- a minimal STM file is already enough to describe source shape, target shape, and field flow
