# TODO: Validator Bugs — False Positives from `satsuma validate`

> **Status: COMPLETED** (2026-03-18). All 5 bugs fixed. `satsuma validate examples/` clean. Remaining false positives against Feature 06 data-modelling examples are tracked in Feature 13.
...
- [ ] Verify `satsuma validate examples/protobuf-to-parquet.stm` produces 0 false positives
...
- [ ] Verify `satsuma validate examples/multi-source-join.stm` field-reference warnings are eliminated
...
- [ ] Verify `satsuma validate examples/metrics.stm` `undefined-ref` warnings reflect actual source names, not `'source'`
...
- [ ] Verify `satsuma validate examples/multi-source-join.stm` `created_at`/`updated_at` warnings are eliminated
...
- [ ] Run `satsuma validate examples/` and confirm 0 errors, 0 false-positive warnings
