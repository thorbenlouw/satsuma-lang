...
1. **Check Python + openpyxl**: Run `python3 -c "import openpyxl"`. If it fails, offer to create a venv:
   ```
   cd features/04-excel-to-stm-skill && python3 -m venv .venv && .venv/bin/pip install -r requirements.txt
   ```
   Then prefix all `python3` commands with `features/04-excel-to-stm-skill/.venv/bin/python3`.
...
Use `features/04-excel-to-stm-skill/excel_tool.py` for ALL Excel access. Never read the Excel file directly.

### Step 1.1 — Tab Inventory

Run:
```bash
python3 features/04-excel-to-stm-skill/excel_tool.py survey <excel-file>
```
...
```bash
python3 features/04-excel-to-stm-skill/excel_tool.py headers <excel-file> "<tab-name>"
python3 features/04-excel-to-stm-skill/excel_tool.py formatting <excel-file> "<tab-name>"
```
...
```bash
python3 features/04-excel-to-stm-skill/excel_tool.py lookup <excel-file> "<tab-name>"
```
...
```bash
python3 features/04-excel-to-stm-skill/excel_tool.py range <excel-file> "<tab>" --rows START:END
```
