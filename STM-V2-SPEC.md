...
STM (Structured Transform Markup) is a concise notation for describing data schemas, mappings, and transformations. It is designed to be read and written by both humans and LLMs.
...
A schema block declares the structure of a data source or target. STM v2 uses a single `schema` keyword — there is no `table`, `message`, `source`, or `target` distinction at the schema level.
